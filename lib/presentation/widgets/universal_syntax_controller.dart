...
class UniversalSyntaxController extends TextEditingController {
  ...
  TextSpan buildTextSpan({
    ...
    for (final match in combinedPattern.allMatches(textStr)) {
      ...
      if (SyntaxPatterns.financePrefix.hasMatch(matchedStr) && match.start == 0) {
        highlightStyle = highlightStyle.copyWith(color: AppColors.finance, fontWeight: FontWeight.bold);
      } else if (SyntaxPatterns.taskPrefix.hasMatch(matchedStr) && match.start == 0) {
        highlightStyle = highlightStyle.copyWith(color: AppColors.tasks, fontWeight: FontWeight.bold);
      } else if (SyntaxPatterns.tags.hasMatch(matchedStr)) {
        highlightStyle = highlightStyle.copyWith(color: AppColors.habits, fontWeight: FontWeight.w600);
      } else if (SyntaxPatterns.mentions.hasMatch(matchedStr)) {
        highlightStyle = highlightStyle.copyWith(color: AppColors.accent, fontWeight: FontWeight.w600);
      } else if (SyntaxPatterns.dateCommands.hasMatch(matchedStr)) {
        highlightStyle = highlightStyle.copyWith(color: AppColors.sleep, fontWeight: FontWeight.w600);
      }
